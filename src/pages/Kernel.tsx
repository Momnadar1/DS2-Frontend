...
function Kernel() {
  ...
  useEffect(() => {
    if (!selectedDataset) {
      axios
        .get("http://localhost:5000/datasets") // 127.0.0.1 l === local
        .then((response) => {
  ...
  const fetchDataset = () => {
    ...
    axios
      .post("http://localhost:5000/datasets", formData)
      .then((response) => {
    ...
    <>
      <div className="pl-20 pt-20 text-white">
        <div className="flex mb-5">
          <img className="w-[60px]" src="vite.svg" alt="" />
          <p className="text-white text-4xl"> &nbsp; Kernel</p>
        </div>
        <div>
          <select
            className="text-black text-xl w-[30%] h-10 rounded-lg bg-slate-200 pl-3 border border-slate-800 mr-5 mb-5"
